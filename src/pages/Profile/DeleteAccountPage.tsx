...
const STYLES = `
  ...
  .pin-no:not(.pushed):hover { transform: rotate(-38deg); }
`;
...
export default function DeleteAccountPage() {
  ...
  const handleDelete = async () => {
    ...
    setError(null);
    try {
      // authApi.deleteAccount(password) sends { data: { password } } on DELETE
      // and calls clearTokens() automatically on success
      await authApi.deleteAccount(password);
      setStep(3);
      setTimeout(() => navigate("/login"), 3000);
    } catch (err: unknown) {
      const e = err as { response?: { data?: { detail?: string; password?: string[] } } };
      const msg =
      ...
      <div style={{ maxWidth: 560, margin: "0 auto" }}>
        ...
        <div style={{ animation: "fadeUp 0.32s ease both", marginBottom: 28 }}>
          <div style={{
            ...
            <span style={{ fontSize: 12, fontWeight: 700, color: DANGER, letterSpacing: "0.5px" }}>DANGER ZONE</span>
          </div>
  ...
  return (
    <div style={{ animation: "fadeUp 0.38s ease both" }}>
      ...
      <div style={{
        ...
        <div>
          <p style={{ margin: "0 0 4px", fontWeight: 800, color: DANGER, fontSize: 14 }}>PERMANENT ACTION</p>
      ...
      <div style={{
        ...
        <div style={{ padding: "14px 20px", borderBottom: "1px solid #f3f4f6", display: "flex", alignItems: "center", gap: 8 }}>
          ...
          <span style={{ fontSize: 12, fontWeight: 700, color: "#9ca3af", letterSpacing: 1 }}>WHAT WILL HAPPEN</span>
        </div>
...
function Step2({
  yesChecked, setYesChecked,
  inputValue, setInputValue,
  password, setPassword,
  timer, loading, error, shake,
  onDelete, onBack,
}: Step2Props) {
  ...
  // All 4 gates must pass before delete is enabled
  const canDelete =
    yesChecked &&
  ...
  return (
    <div style={{ animation: "fadeUp 0.38s ease both" }}>
      ...
      {/* YES / NO card */}
      <div style={{ animation: shake ? "shake 0.45s ease" : "none", marginBottom: 24 }}>
        <div style={{
          background: "#fff", borderRadius: 22,
          border: `1.5px solid ${yesChecked ? DANGER + "55" : "#e5e7eb"}`,
          boxShadow: yesChecked ? `0 4px 24px ${DANGER}18` : "0 2px 12px rgba(0,0,0,0.05)",
          overflow: "hidden",
        ...
        <p style={{ textAlign: "center", fontSize: 12, color: yesChecked ? DANGER : "#9ca3af", marginTop: 10, fontWeight: 600, transition: "color 0.2s" }}>
          {yesChecked ? "⚠️ You selected YES — complete the fields below" : "Click the YES pin to confirm your intent"}
        </p>
      </div>
        ...
        <div style={{ position: "relative" }}>
          <input
            ...
            style={{
              ...
              background: "#f9fafb",
              border: `1.5px solid ${inputValue === CONFIRM_PHRASE ? DANGER + "88" : inputValue.length > 0 ? "#fca5a5" : "#e5e7eb"}`,
              borderRadius: 10,
      ...
      {/* password field — required by backend DeleteAccountSerializer */}
      <div style={{
        background: "#fff", borderRadius: 18,
        border: `1.5px solid ${pwFocused ? DANGER + "88" : password.length > 0 ? DANGER + "44" : "#e5e7eb"}`,
        padding: "20px 22px", marginBottom: 16,
        boxShadow: "0 2px 12px rgba(0,0,0,0.04)",
        transition: "border-color 0.2s",
      }}>
        <p style={{ fontSize: 13, color: "#4b5563", margin: "0 0 12px", fontWeight: 600 }}>
        ...
        <div style={{ position: "relative" }}>
          ...
          <button
            ...
            style={{
              ...
              fontFamily: "'Plus Jakarta Sans', sans-serif", padding: "4px",
            }}
          >
            {showPw ? "HIDE" : "SHOW"}
          </button>
      ...
      <div style={{ display: "flex", gap: 10 }}>
        <button
          ...
          style={{
            ...
            background: canDelete ? (hoverDel ? DANGER_DARK : DANGER) : DANGER_LIGHT,
    ...
    <div style={{
      ...
      >
        ...
        <span style={{
          ...
          marginTop: 4, transition: "color 0.3s", letterSpacing: "0.5px",
        }}>YES</span>
      </div>
      ...
      <div style={{
        ...
        width: 6, height: 6, borderRadius: "50%", background: "#e5e7eb",
      }} />
...
function PinYes({ active }: { active: boolean }) {
  return (
    <svg width="56" height="90" viewBox="0 0 43 90" fill="none" xmlns="http://www.w3.org/2000/svg" style={{ display: "block", margin: "0 auto" }}>
      <rect x="18" y="4" width="7" height="82" rx="3.5" fill={active ? DANGER : "#00A3FF"} style={{ transition: "fill 0.3s" }} />
      <rect x="17.5" y="3.5" width="8" height="83" rx="4" fill="none" stroke={active ? DANGER_DARK : PRIMARY_DARK} strokeWidth="1" style={{ transition: "stroke 0.3s" }} />
      <ellipse cx="21.5" cy="22" rx="18" ry="18" fill={active ? DANGER : "#00A3FF"} style={{ transition: "fill 0.3s" }} />
      <ellipse cx="21.5" cy="22" rx="18" ry="18" fill="none" stroke={active ? DANGER_DARK : PRIMARY_DARK} strokeWidth="1.2" style={{ transition: "stroke 0.3s" }} />
      <rect x="3.5" y="18" width="36" height="5" rx="2" fill={active ? "#fca5a5" : "#7dd3fc"} style={{ transition: "fill 0.3s" }} />
      <ellipse cx="14" cy="13" rx="4" ry="3" fill="rgba(255,255,255,0.35)" transform="rotate(-20 14 13)" />
      <text x="21.5" y="28" textAnchor="middle" fontSize="10" fontWeight="800" fill="#fff" fontFamily="system-ui,sans-serif">YES</text>
    </svg>
  );
}
...
function PinNo() {
  return (
    <svg width="56" height="90" viewBox="0 0 42 89" fill="none" xmlns="http://www.w3.org/2000/svg" style={{ display: "block", margin: "0 auto" }}>
      ...
      <text x="21" y="28" textAnchor="middle" fontSize="10" fontWeight="800" fill="#fff" fontFamily="system-ui,sans-serif">NO</text>
    </svg>
  );
}
  ...
  return (
    <div style={{
      ...
      <p style={{ color: "#6b7280", fontSize: 14, margin: "0 0 20px", lineHeight: 1.6 }}>
        Your account has been permanently removed.<br />We're sorry to see you go.
      </p>
      <div style={{
        display: "inline-flex", alignItems: "center", gap: 8,
